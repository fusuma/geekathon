    ...
    <body>
            ...
            ${Object.entries(label.labelData.legalLabel.nutrition)
              .map(([key, value]) => `
                <tr>
                  ...
                  <td>${value?.per100g?.value}${value?.per100g?.unit}</td>
                </tr>
              `).join('')}
          </tbody>
        </table>
      ...
      ${label.labelData.warnings.length > 0 ? `
        <div class="section warning">
          <h2>Warnings</h2>
          <ul>
            ${label.labelData.warnings.map(w => `<li>${w}</li>`).join('')}
          </ul>
        </div>
      ` : ''}
      ...
      ${label.labelData.complianceNotes.length > 0 ? `
        <div class="section compliance">
          ...
          <ul>
            ${label.labelData.complianceNotes.map(n => `<li>${n}</li>`).join('')}
          </ul>
        </div>
      ` : ''}

      <div class="section">
        <small><strong>Label ID:</strong> ${label.labelId} | <strong>Generated by:</strong> ${label.generatedBy}</small>
      </div>
    </body>
    </html>
  `;
